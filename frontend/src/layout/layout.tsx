...
import { LayoutDashboard, Power, FileText, FolderOpen, CalendarRange, UserCog2, BarChart3, Settings, Building, School, BookCopy, Shield, Menu, X as CloseIcon } from 'lucide-react';
...
const SidebarItem = ({ name, path, icon: Icon, collapsed, isFirst, isLast }: { name: string, path: string, icon: any, collapsed: boolean, isFirst?: boolean, isLast?: boolean }) => {
    ...
    return (
        <NavLink
            ...
            className={`relative flex items-center h-[48px] mx-1.5 ${roundedClass} text-sm font-bold transition-all duration-300 border-2 overflow-hidden ${
                isActive
...
export const Layout = () => {
  ...
  const navItems = [
    { to: "/", label: "Dashboard", icon: LayoutDashboard, roles: ['admin', 'teacher', 'student'], section: 'General' },
    ...
    ...(user?.is_superuser || user?.role === 'superadmin'
      ? [{ to: "/superadmin", label: "Super Admin", icon: Shield, roles: [user.role], section: 'System' }]
      : []),
  ...
  return (
    <div className="bg-bg font-sans flex flex-col h-screen overflow-hidden">
      {/* Premium Top Header */}
      <header className="sticky top-0 z-10 w-full bg-surface border-b-2 border-light/10 px-[2.5%] h-[70px] flex items-center justify-between">

        {/* Left: Logo & Mobile Toggle */}
        <div className="flex-1 flex justify-start items-center gap-4">
          <button
            onClick={handleMobileToggle}
            className="lg:hidden p-2 text-primary hover:bg-primary/10 rounded-full transition-all border border-primary/20"
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {isSidebarOpen && (
          <div
            onClick={handleMobileToggle}
            className="lg:hidden absolute inset-0 bg-bg/80 backdrop-blur-sm z-10"
          />
        )}

        {/* Sidebar Structure Wrapper */}
        <div className="absolute top-4 sm:top-25 bottom-4 left-4 z-50 flex flex-col items-start gap-4 pointer-events-none">
