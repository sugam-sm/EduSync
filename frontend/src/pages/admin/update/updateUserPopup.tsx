...
import {X, GraduationCap, Mail, Phone, UserRound, VenusAndMars, BookOpen, School, HeartHandshake, Link, KeyRound } from "lucide-react";
          ...
          <div className="px-8 pb-8 space-y-8 overflow-y-auto md:overflow-y-visible flex-1">
            <div className="flex items-center gap-4 py-2 border-b border-light/10 pb-4">
              ...
              <button type="button" onClick={() =>
                ...
                className={`w-14 h-7 flex items-center rounded-full p-1 transition-colors duration-300 hover:cursor-pointer ${formData.is_active ? "bg-success/50" : "bg-failure"
                  }`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${formData.is_active ? "translate-x-7" : "translate-x-0"
                    }`}
                />
              </button>

              <span
                className={`font-bold text-sm ${formData.is_active ? "text-success" : "text-failure"
                  }`}
              >
